...
export default function App() {
  ...
  function persistModules(arr) {
    if (hasStorage) window.storage.set("tb_curriculum", JSON.stringify(arr)).catch(() => {});
  }
  function persistDoubts(arr) {
    if (hasStorage) window.storage.set("tb_doubts", JSON.stringify(arr)).catch(() => {});
  }
  ...
  useEffect(() => {
    (async () => {
      ...
      if (hasStorage) {
        try { const r = await window.storage.get("tb_curriculum"); if (r && r.value) mods = JSON.parse(r.value); } catch (e) {}
        try { const r = await window.storage.get("tb_doubts"); if (r && r.value) dbts = JSON.parse(r.value); } catch (e) {}
      }
  ...
  function resetAll() {
    if (typeof window !== "undefined" && window.confirm &&
      !window.confirm("Reset progress, added lessons, and questions back to the starting curriculum?")) return;
    const s = makeSeed();
    updateModules(() => s);
    updateDoubts(() => []);
